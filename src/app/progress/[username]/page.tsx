...
export default function UserProgressPage() {
    ...
    const calculateStreak = (logs: TimelineItem[]) => {
        ...
        // Check if today is in the logs
        const todayIndex = uniqueDays.indexOf(today.toISOString());
        if (todayIndex === -1) {
            // Check if yesterday is in the logs. If not, streak is 0.
        ...
        for (let i = 1; i < uniqueDays.length; i++) {
            ...
            if (differenceInDays(lastDay, currentDay) === 1) {
                currentStreak++;
                lastDay = currentDay;
            } else {
                break;
    ...
    if (!username) {
         return (
            <div className="flex min-h-screen items-center justify-center bg-[#0F0F0F] text-white">
                <p>Could not load progress. Please log in again.</p>
            </div>
        );
    }
            ...
            <div className="container mx-auto p-4">
                ...
                <Card className="bg-[#1A1A1A] border-[#2A2A2A] text-white mb-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <Button onClick={handleAnalysis} disabled={isAnalyzing} className="w-full bg-primary hover:bg-primary/90 text-lg py-6">
