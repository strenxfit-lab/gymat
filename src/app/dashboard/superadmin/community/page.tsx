...
import { Loader2, Send, Plus, Image as ImageIcon, Video, X, ThumbsUp, MessageSquare, MoreVertical, Flag, Repeat, Share2, Search, User, Rss, LayoutDashboard, Edit, Trash, Activity, BarChart } from "lucide-react";
...
export default function SuperAdminCommunityPage() {
  ...
  const repostForm = useForm<RepostFormData>({
    ...
    defaultValues: { caption: '', visibility: "global" },
  });
  ...
  useEffect(() => {
    if (editingPost) {
        ...
        postForm.reset({ text: '', visibility: 'global', mediaUrls: []});
        setMediaPreviews([]);
    }
  ...
  const handlePostSubmit = async (values: PostFormData) => {
    ...
    const gymId = "superadmin_gym";
    const authorName = localStorage.getItem('communityUsername') || "Super Admin";
  ...
  const handleShareToChat = async (chatId: string) => {
      ...
      const message = {
          ...
          senderId: localStorage.getItem('userDocId'),
