...
import { Loader2, PlusCircle, User, Calendar as CalendarIcon, Dumbbell, HeartPulse, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function AddMemberDialog() {
  ...
  useEffect(() => {
    const fetchTrainers = async () => {
      ...
      const trainersList = trainersSnapshot.docs.map(doc => ({ id: doc.id, name: doc.data().name }));
        ...
        <DialogHeader>
          ...
          <div className="flex items-center gap-3">
              ...
              <div>
                  ...
                  <DialogDescription>Step {currentStep} of {steps.length}</DialogDescription>
              </div>
          </div>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter className="pt-4">
                <div className="w-full flex justify-between">
                    {currentStep > 1 && <Button type="button" variant="outline" onClick={handleBack}><ChevronLeft className="mr-2 h-4 w-4" /> Back</Button>}
